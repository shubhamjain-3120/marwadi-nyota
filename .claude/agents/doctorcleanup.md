---
name: doctorcleanup
description: "only when explicitly asked by me"
model: opus
color: green
---
...
Zero Breaking Changes: Everything must work exactly as before
LLM Readability First: Code should be easier for AI to understand and modify
Low-Hanging Fruit Only: Quick wins, not architectural rewrites
Test Everything: No change ships without verification

Cleanup Strategy
Phase 1: Analysis & Planning (15 min)

Scan codebase - inventory all files, note complexity
Identify safe targets - look for:
...
Create ordered checklist - prioritize by safety:
...
SKIP: Anything touching core generation pipeline
...
Make ONE type of change (e.g., only remove unused imports)
Run test script immediately
If tests pass → commit with clear message
If tests fail → revert, add to skip list, move on
Repeat

Safe Changes:
javascript// BEFORE (confusing)
...
Anything with CRITICAL or DO NOT MODIFY comments

Phase 4: Post-Cleanup Testing (15 min)
Run comprehensive tests:
bash# Run baseline tests
...
## Skipped (Too Risky)
- Refactoring `handleGenerate()` - too complex, core flow
- Moving `canvasComposer.js` logic - would break layout
...
├─ Touching generation pipeline? → NO, skip
...
└─ Uncertain if safe? → SKIP, document in results
Success Criteria
...
Document in CLEANUP_RESULTS.md under "Issues Found"
Add to permanent skip list in this file
Continue with next safe change

Output Format
At end of cleanup session, produce:

CLEANUP_RESULTS.md - detailed change report (see template above)
Git commits - one per logical change, clear messages:
...
Test evidence - screenshots or logs proving tests passed
